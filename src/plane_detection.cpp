...
bool
PlaneDetection::readColorImage(string filename)
{
    color_img_ = cv::imread(filename, CV_LOAD_IMAGE_COLOR);
    if (color_img_.empty() || color_img_.depth() != CV_8U)
    {
	cout << "ERROR: cannot read color image. No such a file, or the image format is not 8UC3"
	     << endl;
	return false;
    }
    return true;
...
bool
PlaneDetection::readDepthImage(string filename)
{
    cv::Mat depth_img = cv::imread(filename, CV_LOAD_IMAGE_ANYDEPTH);
    if (depth_img.empty() || depth_img.depth() != CV_16U)
    {
	cout << "WARNING: cannot read depth image. No such a file, or the image format is not 16UC1"
	     << endl;
	return false;
    }
	...
	{
	    double z = (double)(depth_img.at<unsigned short>(i, j))
		     / kScaleFactor;
	    if (_isnan(z))
	    {
...
bool
PlaneDetection::runPlaneDetection()
{
...
PlaneDetection::prepareForMRF()
{
	...
	for (int col = 0; col < kDepthWidth; ++col)
	{
		...
		for (int x = max(row - kNeighborRange, 0);
		     x < min(kDepthHeight, row + kNeighborRange); ++x)
		{
		    for (int y = max(col - kNeighborRange, 0);
			 y < min(kDepthWidth, col + kNeighborRange); ++y)
		    {
	...
	cv::Vec3b c = seg_img_.at<cv::Vec3b>(vidx / kDepthWidth,
					     vidx % kDepthWidth);
...
void
PlaneDetection::writeOutputFiles(string output_folder,
				 string frame_name, bool run_mrf)
{
...
void
PlaneDetection::writePlaneLabelFile(string filename, bool run_mrf /*=false */)
{
	...
	for (int col = 0; col < kDepthWidth; ++col)
	{
	    int label = run_mrf ? opt_membership_img_.at<int>(row, col)
				: plane_filter.membershipImg.at<int>(row, col);
	    out << label << " ";
	}
	out << endl;
    }
...
void
PlaneDetection::writePlaneDataFile(string filename, bool run_mrf /* = false */)
{
    ofstream out(filename, ios::out);
...
void
PlaneDetection::computePlaneSumStats(bool run_mrf /* = false */)
{
	...
	for (int i = 0; i < plane_vertices_[pidx].size(); ++i)
	{
	    ...
	    sum_stats_[pidx].sx += v[0];
	    sum_stats_[pidx].sy += v[1];
	    sum_stats_[pidx].sz += v[2];
	    sum_stats_[pidx].sxx += v[0] * v[0];
	    sum_stats_[pidx].syy += v[1] * v[1];
	    sum_stats_[pidx].szz += v[2] * v[2];
	    sum_stats_[pidx].sxy += v[0] * v[1];
	    sum_stats_[pidx].syz += v[1] * v[2];
	    sum_stats_[pidx].sxz += v[0] * v[2];
	}
	...
	sum_stats_[pidx].sx /= num;
	sum_stats_[pidx].sy /= num;
	sum_stats_[pidx].sz /= num;
	sum_stats_[pidx].sxx /= num;
	sum_stats_[pidx].syy /= num;
	sum_stats_[pidx].szz /= num;
	sum_stats_[pidx].sxy /= num;
	sum_stats_[pidx].syz /= num;
	sum_stats_[pidx].sxz /= num;
    }
  ...
  // So here we compute a mapping between these two types of plane indices by comparing plane centers.
    ...
    {
	sx[i] = plane_filter.extractedPlanes[i]->stats.sx
	      / plane_filter.extractedPlanes[i]->stats.N;
	sy[i] = plane_filter.extractedPlanes[i]->stats.sy
	      / plane_filter.extractedPlanes[i]->stats.N;
	sz[i] = plane_filter.extractedPlanes[i]->stats.sz
	      / plane_filter.extractedPlanes[i]->stats.N;
    }
  ...
  // If two planes' centers are closest, then the two planes are corresponding to each other.
	...
	{
	    double	a = sum_stats_[i].sx - sx[j],
			b = sum_stats_[i].sy - sy[j],
			c = sum_stats_[i].sz - sz[j];
	...
	for (int row = 0; row < kDepthHeight; ++row)
	{
		...
		if (label != plane_num_) // pixel belongs to some plane
		{
		    ...
		    const VertexType& v = cloud.vertices[vidx];
		    opt_sum_stats_[label].sx += v[0];
		    opt_sum_stats_[label].sy += v[1];
		    opt_sum_stats_[label].sz += v[2];
		    opt_sum_stats_[label].sxx += v[0] * v[0];
		    opt_sum_stats_[label].syy += v[1] * v[1];
		    opt_sum_stats_[label].szz += v[2] * v[2];
		    opt_sum_stats_[label].sxy += v[0] * v[1];
		    opt_sum_stats_[label].syz += v[1] * v[2];
		    opt_sum_stats_[label].sxz += v[0] * v[2];
		}
	    }
	}
	for (int pidx = 0; pidx < plane_num_; ++pidx)
	{
	    ...
	    opt_sum_stats_[pidx].sx /= num;
	    opt_sum_stats_[pidx].sy /= num;
	    opt_sum_stats_[pidx].sz /= num;
	    opt_sum_stats_[pidx].sxx /= num;
	    opt_sum_stats_[pidx].syy /= num;
	    opt_sum_stats_[pidx].szz /= num;
	    opt_sum_stats_[pidx].sxy /= num;
	    opt_sum_stats_[pidx].syz /= num;
	    opt_sum_stats_[pidx].sxz /= num;
	}
    }
  ...
  // Only for debug. It doesn't influence the plane detection.
	...
	w -= plane_filter.extractedPlanes[pidx]->normal[0]
	   * sum_stats_[pidx].sx;
	w -= plane_filter.extractedPlanes[pidx]->normal[1]
	   * sum_stats_[pidx].sy;
	w -= plane_filter.extractedPlanes[pidx]->normal[2]
	   * sum_stats_[pidx].sz;
	double sum = 0;
	for (int i = 0; i < plane_vertices_[pidx].size(); ++i)
	{
