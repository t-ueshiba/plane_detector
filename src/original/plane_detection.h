...
class PlaneDetection
{
  ...
  //bool readIntrinsicParameterFile(string filename);
    ...
    void	prepareForMRF();
    void	writeOutputFiles(string output_folder,
				 string frame_name, bool run_mrf = false);
  ...
  /* For MRF optimization */
    inline MRF::CostVal
    dCost(int pix, int label)
    {
	return (pixel_boundary_flags_[pix] ? 1 :
		(label == plane_filter.membershipImg.at<int>(pix/kDepthWidth,
							     pix%kDepthWidth) ?
		 1 : kInfVal));
    }

    inline MRF::CostVal
    fnCost(int pix1, int pix2, int i, int j)
    {
